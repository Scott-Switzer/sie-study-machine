var SIE_QUESTIONS_YT_GURU14 = [
 {
  "id": "yt-guru14-11",
  "section": "3 - Trading, Accounts & Prohibited",
  "stem": "An associated person is unable to work for any business other than his member firm without having the employing broker-dealer's",
  "choices": {
   "A": "authorization by a principal.",
   "B": "assuming liability for that business.",
   "C": "knowledge of the activity.",
   "D": "permission in writing."
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cauthorization by a principal.\u201d. This falls under <b>3 - Trading, Accounts & Prohibited</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-12",
  "section": "1 - Capital Markets",
  "stem": "All of the following must be on the cover page or beginning of the summary prospectus of a mutual fund except",
  "choices": {
   "A": "the share class or classes offered by the fund.",
   "B": "the fund share class or classes ticker symbols.",
   "C": "the website where a full prospectus may be downloaded.",
   "D": "the name of the investment advisor."
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cthe share class or classes offered by the fund.\u201d. This falls under <b>1 - Capital Markets</b>. Key principle: The prospectus is the formal offer document; the red herring is pre-effective.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-14",
  "section": "3 - Trading, Accounts & Prohibited",
  "stem": "As long as it is not restricted in any documentation, margin trading is permissible in which of the following accounts? I. Corporate II. IRA III. Partnership IV. Fiduciary",
  "choices": {
   "A": "I and IV",
   "B": "II and IV",
   "C": "I and III",
   "D": "II and III"
  },
  "answer": "C",
  "explanation": "The correct choice (C) is: \u201cI and III\u201d. This falls under <b>3 - Trading, Accounts & Prohibited</b>. Key principle: Initial margin 50%; maintenance 25% (FINRA Reg T / FINRA 4210).",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-15",
  "section": "2 - Products & Risks",
  "stem": "A customer buys a callable 5% coupon bond at par that will mature in 10 years. Which of the following statements is true?",
  "choices": {
   "A": "Yield to call (YTC) is lower than yield to maturity (YTM).",
   "B": "Nominal yield is higher than either yield to maturity (YTM) or yield to call (YTC).",
   "C": "Yield to call (YTC) is higher than yield to maturity (YTM).",
   "D": "Yield to call (YTC) is the same as yield to maturity (YTM)."
  },
  "answer": "D",
  "explanation": "The correct choice (D) is: \u201cYield to call (YTC) is the same as yield to maturity (YTM).\u201d. This falls under <b>2 - Products & Risks</b>. Key principle: Yield is the return on a security; price and yield move inversely for bonds.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-16",
  "section": "3 - Trading, Accounts & Prohibited",
  "stem": "A customer has a significant amount of money in bank deposit accounts: $225,000 in a savings account titled in the customer's name; $240,000 in a checking account titled jointly with a spouse; and $100,000 in an account where the customer is custodian for a grandchild. Should that bank fail, the Federal Deposit Insurance Corporation (FDIC) insurance would cover",
  "choices": {
   "A": "the entire $565,000.",
   "B": "a total of $250,000, divided proportionately among the three accounts.",
   "C": "$250,000 for the savings and checking accounts and $100,000 for the custodial account.",
   "D": "$225,000 for the savings account, $100,000 for the custodial account, and nothing for the checking account."
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cthe entire $565,000.\u201d. This falls under <b>3 - Trading, Accounts & Prohibited</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-17",
  "section": "3 - Trading, Accounts & Prohibited",
  "stem": "Which of the following best describes how a sell stop at 39 order would be filled?",
  "choices": {
   "A": "The next price above 39 after the market rises to 39",
   "B": "The next available price after the market price falls to 39",
   "C": "The next available price after the market price rises to 39",
   "D": "The next price below 39 after the market falls to 39"
  },
  "answer": "D",
  "explanation": "The correct choice (D) is: \u201cThe next price below 39 after the market falls to 39\u201d. This falls under <b>3 - Trading, Accounts & Prohibited</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-26",
  "section": "2 - Products & Risks",
  "stem": "Which of the following are considered tools used to implement fiscal policies?",
  "choices": {
   "A": "I and IV",
   "B": "II and III",
   "C": "I and III",
   "D": "II and IV"
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cI and IV\u201d. This falls under <b>2 - Products & Risks</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-32",
  "section": "3 - Trading, Accounts & Prohibited",
  "stem": "In order for a registered representative of a member firm to receive any form of compensation, such as commissions, after terminating employment, all of the following statements are correct except",
  "choices": {
   "A": "the agreement must be entered into before the termination of employment.",
   "B": "it would be permissible to pay continuing commissions to a surviving spouse.",
   "C": "there must be a contract in effect calling for these continuing commissions.",
   "D": "earnings from referred business from existing clients would be eligible for payment."
  },
  "answer": "D",
  "explanation": "The correct choice (D) is: \u201cearnings from referred business from existing clients would be eligible for payment.\u201d. This falls under <b>3 - Trading, Accounts & Prohibited</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-37",
  "section": "2 - Products & Risks",
  "stem": "An investor is convinced that CDT stock will soon decline in value for a number of reasons. Which investment strategy will allow the investor to take advantage of the anticipated decline in share value with the smallest cash investment?",
  "choices": {
   "A": "Purchase a call spread",
   "B": "Purchase a put option",
   "C": "Purchase a call option",
   "D": "Sell the company's stock short"
  },
  "answer": "B",
  "explanation": "The correct choice (B) is: \u201cPurchase a put option\u201d. This falls under <b>2 - Products & Risks</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-38",
  "section": "4 - Regulatory Framework",
  "stem": "A registered representative has left one firm to join another. Sometime later, the former employer discovers that some information on Form U5 filed at the time of termination was inaccurate. The firm need not file an amended U5 if at least how much time has gone by?",
  "choices": {
   "A": "No stated time limit",
   "B": "5 years",
   "C": "2 years",
   "D": "30 days"
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cNo stated time limit\u201d. This falls under <b>4 - Regulatory Framework</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-43",
  "section": "4 - Regulatory Framework",
  "stem": "Yukon Gold, Inc., has secured rights to develop a rich vein of precious metals in a foreign country. Shortly after the facility begins production, the government of that nation collapses and the new government is insisting on renegotiating the contract with the company. This is an example of",
  "choices": {
   "A": "business risk.",
   "B": "legislative risk.",
   "C": "regulatory risk.",
   "D": "political risk."
  },
  "answer": "D",
  "explanation": "The correct choice (D) is: \u201cpolitical risk.\u201d. This falls under <b>4 - Regulatory Framework</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-104",
  "section": "1 - Capital Markets",
  "stem": "A company is considering raising capital without going through the registration process requirements mandated by the Securities Act of 1933. To be exempt from the act, which of the following offerings might they employ?",
  "choices": {
   "A": "Additional public offering (APO)",
   "B": "Private (nonpublic) securities offering",
   "C": "Shelf offering",
   "D": "Initial public offering"
  },
  "answer": "B",
  "explanation": "The correct choice (B) is: \u201cPrivate (nonpublic) securities offering\u201d. This falls under <b>1 - Capital Markets</b>. Key principle: New issues register with the SEC via registration; exempt securities (e.g. munis) do not require it.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-108",
  "section": "2 - Products & Risks",
  "stem": "MNO High Yield Bond Fund has a public offering price (POP) of $120 per share and a net asset value (NAV) of $116.40. What is the fund's sales charge (SC)?",
  "choices": {
   "A": "5%",
   "B": "2%",
   "C": "3%",
   "D": "6%"
  },
  "answer": "C",
  "explanation": "The correct choice (C) is: \u201c3%\u201d. This falls under <b>2 - Products & Risks</b>. Key principle: Yield is the return on a security; price and yield move inversely for bonds.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-109",
  "section": "2 - Products & Risks",
  "stem": "Different categories of preferred shares offered by an issuer",
  "choices": {
   "A": "all have preference over the issuer's common shares.",
   "B": "all must have the same fixed dividend rate.",
   "C": "must all be callable shares.",
   "D": "must all be convertible shares."
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201call have preference over the issuer's common shares.\u201d. This falls under <b>2 - Products & Risks</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-111",
  "section": "2 - Products & Risks",
  "stem": "An investor owns 3% preferred stock participating to 6%. This means the investor",
  "choices": {
   "A": "could receive an additional 3% over the stated 3% dividend if the board declares it.",
   "B": "could receive an additional 6% over the stated 3% dividend.",
   "C": "must receive at least 6% each year.",
   "D": "must receive a total of 9% in any year the board declares the 6% participating be paid."
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201ccould receive an additional 3% over the stated 3% dividend if the board declares it.\u201d. This falls under <b>2 - Products & Risks</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-112",
  "section": "1 - Capital Markets",
  "stem": "Regarding the registration statement filed with the Securities and Exchange Commission (SEC) when new securities are to be issued, all of the following are true except",
  "choices": {
   "A": "underwriters may assist the issuer in preparing and filing the registration statement.",
   "B": "the names and addresses of company officers and directors, their salaries, and a five-year business history of each must be shown.",
   "C": "the accuracy and adequacy of the registration documents is the responsibility of the underwriters.",
   "D": "a description of how the proceeds raised from the sale will be used must be disclosed."
  },
  "answer": "B",
  "explanation": "The correct choice (B) is: \u201cthe names and addresses of company officers and directors, their salaries, and a five-year business history of each must be shown.\u201d. This falls under <b>1 - Capital Markets</b>. Key principle: New issues register with the SEC via registration; exempt securities (e.g. munis) do not require it.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-113",
  "section": "2 - Products & Risks",
  "stem": "Which of the following rights are specific to holders of American depositary receipts and are not available to owners of domestic common stock?",
  "choices": {
   "A": "The right to pay taxes on received dividends",
   "B": "The right to convert to the underlying asset",
   "C": "The right to transfer the shares",
   "D": "The right to receive dividends"
  },
  "answer": "B",
  "explanation": "The correct choice (B) is: \u201cThe right to convert to the underlying asset\u201d. This falls under <b>2 - Products & Risks</b>. No detailed explanation was captured from the source video; the correct answer reflects the standard SIE treatment for this topic \u2014 review it before the exam.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-118",
  "section": "1 - Capital Markets",
  "stem": "A registration statement for a new debt offering of BuyStuff, Inc., a well-known seasoned issuer, has been filed with the Securities and Exchange Commission (SEC). The accuracy and adequacy of the registration statement is the responsibility of",
  "choices": {
   "A": "the underwriters.",
   "B": "the issuer.",
   "C": "the SEC.",
   "D": "the exchanges where the shares will trade."
  },
  "answer": "C",
  "explanation": "The correct choice (C) is: \u201cthe SEC.\u201d. This falls under <b>1 - Capital Markets</b>. Key principle: New issues register with the SEC via registration; exempt securities (e.g. munis) do not require it.",
  "source": "YouTube: Series 7 Guru (guru14)"
 },
 {
  "id": "yt-guru14-140",
  "section": "1 - Capital Markets",
  "stem": "Primary market transactions would include which of the following?",
  "choices": {
   "A": "Sale of $10 million of corporate bond by a broker-dealer acting as an underwriter",
   "B": "Sale of $10 million of municipal bonds by a broker-dealer acting as a market maker",
   "C": "Sale of $10 million of corporate stock by a broker-dealer acting as a market maker",
   "D": "Sale of $10 million of U.S. Treasury bonds by a broker-dealer acting as a market maker"
  },
  "answer": "A",
  "explanation": "The correct choice (A) is: \u201cSale of $10 million of corporate bond by a broker-dealer acting as an underwriter\u201d. This falls under <b>1 - Capital Markets</b>. Key principle: The primary market is where issuers sell new securities to investors.",
  "source": "YouTube: Series 7 Guru (guru14)"
 }
];
